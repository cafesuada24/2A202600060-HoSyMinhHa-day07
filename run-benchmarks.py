...
for strategy_name, splitter in splitters.items():
    ...
    agg_var = {
        "Strategy": strategy_name,
        # f"Accuracy@1": round(np.var(metrics["acc_1"]), 4),
        f"Recall@{K}": round(np.var(metrics["recall"]), 4),
        f"Precision@{K}": round(np.var(metrics["precision"]), 4),
        # f"F1@{K}": round(np.var(metrics["f1"]), 4),
        f"MRR@{K}": round(np.var(metrics["mrr"]), 4),
        f"MAP@{K}": round(np.var(metrics["map"]), 4),
        f"NDCG@{K}": round(np.var(metrics["ndcg"]), 4),
    }
